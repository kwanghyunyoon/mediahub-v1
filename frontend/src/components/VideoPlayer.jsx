...
export default function VideoPlayer({
  ...
  // Compute source at the top so hooks are never conditional.
  const source = media ? parseSource(media) : null;
  const meta = source ? (SOURCE_META[source.kind] || SOURCE_META.embed) : SOURCE_META.embed;
  const isIframe = source ? source.kind !== "direct" : false;

  // Reset error state when a different media item is opened.
  useEffect(() => {
    setVideoError(false);
    setVideoKey((k) => k + 1);
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      if (e.key === "Escape" && !confirmingDelete) onCloseRef.current?.();
    };
  ...
  // AnimatePresence wraps the conditional so the exit animation fires
  // when media becomes null, instead of returning null immediately.
  return (
    <AnimatePresence>
      {media && source && (
        <motion.div
